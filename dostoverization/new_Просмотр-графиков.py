...
st.markdown(f' чувствительность:  {st.session_state.porog}')
...
if "dir_name" in st.session_state:
    ...
    if st.session_state.button_click:
        if option == "все графики":
            ...
            df2=pd.read_excel(os.path.join(folder_path,'Получасовки_new.xlsx')) # файл после обработки
            df3=pd.read_excel(os.path.join(folder_path,'Отчет_о_достоверизации_new.xlsx')) # файл после обработки
            ...
            # Нахождение строк, содержащих заданное значение
                ...
                fig.update_layout(
                    title={
                        'text': f"{i}, ТУ: {f3.iloc[0,1]}, расхождение: {f3.iloc[0,6]} кВт*ч, чувствительность: {st.session_state.porog}",
                        'xanchor': 'center',
                        'yanchor': 'top',
                        'yref': 'paper',
                        'xref': 'paper',
                ...
                figures.append(fig)
                st.markdown(f"{i}, ТУ: {f3.iloc[0,1]}, расхождение: {f3.iloc[0,6]} кВт*ч, чувствительность: {st.session_state.porog}")
            ...
            df2=pd.read_excel(os.path.join(folder_path, 'Получасовки_new.xlsx'), decimal=',',dtype='str') # файл после обработки
            df3=pd.read_excel(os.path.join(folder_path, 'Отчет_о_достоверизации_new.xlsx'), decimal=',',dtype='str') # файл после обработки
            ...
            # Создаем график
            fig = px.line(ff, x='дата_время', y=['было', 'стало'],title=f'{id}, ТУ: {f3.iloc[0,1]}, d_old:{f3.iloc[0,4]}кВт*ч, чувствительность: {st.session_state.porog}')
